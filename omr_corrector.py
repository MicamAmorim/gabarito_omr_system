...
def make_debug_overlay(warped_bgr: np.ndarray, positions: Dict[int, Dict[str, Tuple[float, float]]], reads: List[BubbleRead]) -> np.ndarray:
    out = warped_bgr.copy()
    status_color = {"ok": (0, 180, 0), "low_confidence": (0, 180, 180), "multiple": (0, 0, 255), "blank": (255, 0, 0)}
    read_by_q = {r.question: r for r in reads}
    for q, alts in positions.items():
        r = read_by_q.get(q)
        color = status_color.get(r.status if r else "blank", (255, 0, 0))
        for alt, (cx, cy) in alts.items():
            cv2.circle(out, (int(cx), int(cy)), 9, color, 1)
        if q % 20 == 1:
            x0, y0 = next(iter(alts.values()))
            cv2.putText(out, str(q), (int(x0) - 50, int(y0) + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.45, color, 1)
    return out
...
def process(input_path: str | Path, config_path: str | Path, output_dir: str | Path, dpi: int = 220, debug: bool = False) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for img, page, src in iter_pages(input_path, dpi=dpi):
        ...
        if debug:
            ...
            cv2.imwrite(str(output_dir / "debug" / f"{Path(src).stem}_p{page:03d}.jpg"), overlay)
            with open(output_dir / "debug" / f"{Path(src).stem}_p{page:03d}.layout.json", "w", encoding="utf-8") as f:
                json.dump({"align_ok": align_ok, "layout_ok": layout_ok, **layout_debug}, f, ensure_ascii=False, indent=2)
